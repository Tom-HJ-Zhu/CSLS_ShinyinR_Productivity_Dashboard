...
server <- function(input, output) {
  ...
  sectors <-reactive({
    ...
    if (is.null(sel) || length(sel) == 0){
      search_for <- c("All industries")
    } else{
      get_first_from_leaves <- function(x) {
        if (is.list(x)) {
          unlist(lapply(x, get_first_from_leaves))
        } else {
          x[1]
        }
      }
      search_for <- get_first_from_leaves(sel)
    }
    search_for
  })
  ...
  df_2plot <- reactive({
    ...
    for (i in search_for) {
      for (j in 1:nrow(df) )
        if (startsWith(df[j, 1], i)) {
          sectors_num <- append(sectors_num,j)
          break
        }
    }
    ...
    t0 <- unlist(sectors_num)+shift+1
    t1 <- unlist(sectors_num)+shift+num_obs
    
    ##get the data we wanted from the csv
    out <- data.frame(matrix(nrow=num_obs, ncol=0))
    out$Years <- horizon
    for (i in 1:length(t0)){
      out[[search_for[i]]] <- as.numeric(df[t0[i]:t1[i],1])
    }
  ...
  output$line <- renderPlotly({
    d <- df_2plot()
    df_long <- pivot_longer(d,cols=-Years, names_to = "Sectors", values_to = "Productivity")
    tit <- paste0("Labor Productivity in ", provinces[as.numeric(input$radio)])
    p <-ggplot(df_long, aes(x = Years, y = Productivity, color = Sectors)) +
  ...
  df_2table <- reactive({
    ...
    #call in inputs
    req(input$cagr_year, input$radio)
    ...
    for (i in 1:length(t0)){
      temp <- as.numeric(df[t1[i],1])/as.numeric(df[t0[i],1])
      temp <- temp^(1/(input$cagr_year[2]-input$cagr_year[1]))-1
      out[[search_for[i]]] <- percent(temp,accuracy=0.01)
    }
